...
export class employeeManageUsers extends React.Component<Props, State> {
  ...
  blockSwitch(e: User) {
    if (e.active == 1) {
      return <Button style={{ 'width': '6em' }} bsSize={'sm'} bsStyle={'danger'}
                     onClick={this.blockUser.bind(null, e.mail)}>Block</Button>;
    } else {
      return <Button style={{ 'width': '6em' }} bsSize={'sm'} bsStyle={'primary'}
                     onClick={this.unblockUser.bind(null, e.mail)}>Unblock</Button>;
    }
  }
