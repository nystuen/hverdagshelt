...
export class NotificationSettingsDao extends Dao {
  ...
  getUsersWithNotificationsLikeThis(countyId: string, categoryId: string, callback: Function) {
    super.query('SELECT userMail FROM pushAlerts WHERE countyid =? AND categoryId=?', [countyId, categoryId], callback);
  }
